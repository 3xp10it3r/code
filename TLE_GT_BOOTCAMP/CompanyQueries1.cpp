...
void dfs(int currentNode, int parent, vector<vector<int>> & adj, vector<int>& firstParents){
    firstParents[currentNode] = parent;
    for(auto neighbour: adj[currentNode]){
        if(neighbour != parent){
            dfs(neighbour, currentNode, adj, firstParents);
        }
    }
}
...
void solve() {
    ...
    while(q){
        ...
        cin >> x >> k;
        x--;
        int kthPar = kthParent(k,x,parents);
        int ans =  kthPar == -1 ? -1 : kthPar + 1 ;
        cout << ans << endl;
        q--;
    }
